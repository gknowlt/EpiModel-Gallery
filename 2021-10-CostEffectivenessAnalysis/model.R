...
cost <- c(
  mean(colSums(sim_bl$epi$cost.disc, na.rm = TRUE)),
  mean(colSums(sim_inter$epi$cost.disc, na.rm = TRUE))
)
effect <- c(
  mean(colSums(sim_bl$epi$qaly.disc, na.rm = TRUE)),
...
calc_outcomes <- function(sim, intervention) {
  ...
  if (intervention == TRUE) {
    inter.cost.vec <- c(
      rep(
        inter.cost / (end.horizon - cea.start),
...
cost <- c(
  calc_outcomes(sim = sim_bl, intervention = FALSE)$cuml.cost.disc,
  calc_outcomes(sim = sim_inter, intervention = TRUE)$cuml.cost.disc
)
effect <- c(
  calc_outcomes(sim = sim_bl, intervention = FALSE)$cuml.qaly.disc,
